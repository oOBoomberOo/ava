...
ipc.options.then(async options => {
	...
	try {
		...
		// Wrap the actual `process.exit` function so that it can be report when the test try to use `process.exit`
		process.exit = (code = 0) => {
			if (code !== 0) {
				const error = new Error(`Test forcibly exit with code status: ${code}`);
				ipc.send({type: 'uncaught-exception', err: serializeError('Forcibly exit', true, error, runner.file)});
			}

			oldExit.apply(process, [code]);
		};
